...
#' @importFrom dplyr select mutate group_by summarise arrange distinct all_of across everything left_join case_when
#' @importFrom tidyr replace_na nest unnest pivot_longer pivot_wider
#' @importFrom crayon green bold italic red blue underline yellow
    ...
    tryCatch({
      if(method2 == "pls-da") {
        tmp_x = ropls::opls(
          ...
          ) %>% as.matrix(),fig.pdfC = 'none',info.txtC = 'none',
          crossvalI = crossv)
